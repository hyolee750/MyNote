...
确认类型符合Java语言的语义，并且它不会危害虚拟机的完整性

首先列出确保各个类之间的二进制兼容的检查：

	1. 检查final类不能拥有于类
	2. 检查final的方法不能被覆盖
	3. 确保在类型和超类型之间没有不兼容的方法声明
	
这个类和它所有的超类型都需要确保互相之间仍然二进制兼容：
	
	1. 检查所有的常量池入口互相之间一致
	2. 检查常量池中的所有的特殊字符串，类名，字段名，方法名和字段描述符，方法描述符
	3. 检查字节码的完整性
	
#### 准备

在准备阶段，Java虚拟机为**类变量分配内存**，**设置默认初始值**
但是在到达初始化之前，类变量都没有被初始化为真正的值。在准备阶段是不会执行Java代码的。
...
初始化一个类包含两个步骤：
	...
	1. 如果类存在直接超类的话，且直接超类还没有初始化，就先初始化直接超类
