...
| 选项   | 功能                               |
| ---- | -------------------------------- |
| -f   | 设置在进行解压缩时，如果输出的文件与现有文件同名，则进行覆盖操作 |
| -k   | 在解压缩完成后，设置保留原始的压缩文件              |
| -s   | 在程序运行时设置尽量减少程序运行的内存使用量           |
...
- 双击`g`跳转到文本文件的开始
- 双击`G`跳转到文本文件的末尾
- 使用`/`进行搜索，从当前光标所在位置开始搜索，使用`n`重复执行搜索，使用`N`进行反向搜索
- 使用`？`进行搜索，从当前光标所在位置向上搜索，使用n重复执行搜索，使用N进行反向搜索
